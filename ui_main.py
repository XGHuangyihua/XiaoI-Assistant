...
class XiaoIWindow(QMainWindow):
    ...
    def send_task_qianwen(self, prompt):
        ...
        # ========== 步骤1: 全屏模板匹配找对话区域 ==========
        self.log("📋 步骤1: 全屏模板匹配查找对话区域...")
        chat_thresholds = [0.15, 0.2, 0.25, 0.3]
        chat_area = None

        for thresh in chat_thresholds:
            self.log(f"  🔍 尝试阈值 {thresh}...")
            chat_area = self.recognizer.find_template(
                "qianwen_chat.jpeg", confidence=thresh
            )
            if chat_area and "center" in chat_area:
                self.log(
                    f"  ✅ 找到对话区域! 位置: {chat_area['center']}, 尺寸: {chat_area['size']}, 匹配度: {chat_area['confidence']:.3f}"
                )
                break
            time.sleep(0.1)

        if not chat_area or "center" not in chat_area:
            self.log("❌ 步骤1失败: 未找到对话区域")
            self.log("❌ 任务中止 - 无法定位对话区域")
            return

        # 验证匹配度是否足够高
        if chat_area["confidence"] < 0.4:
            self.log(
                f"❌ 步骤1失败: 对话区域匹配度过低 ({chat_area['confidence']:.3f} < 0.4)"
            )
            self.log("❌ 任务中止 - 请确保千问页面正常显示")
            return

        # 鼠标移到对话区域中心
        chat_cx, chat_cy = chat_area["center"]
        self.log(f"  🖱️  鼠标移动到对话区域中心 ({chat_cx}, {chat_cy})...")
        pyautogui.moveTo(chat_cx, chat_cy)
        self.log("  ✅ 已移动到对话区域中心")
        time.sleep(0.5)

        # ========== 步骤2: 在对话区域内动态匹配输入框 ==========
        self.log("📋 步骤2: 在对话区域内动态匹配输入框...")

        chat_top_left = chat_area["top_left"]
        chat_size = chat_area["size"]

        search_region = {"top_left": chat_top_left, "size": chat_size}

        self.log(f"  📍 对话区域: 左上{chat_top_left}, 尺寸{chat_size}")
        ...
        for thresh in input_thresholds:
            self.log(f"  🔍 尝试阈值 {thresh}...")
            input_box = self.recognizer.find_template_in_region(
                "qianwen_input_box.jpeg", search_region, confidence=thresh
            )
        ...
        if input_box["confidence"] < 0.4:
            self.log(
                f"❌ 步骤2失败: 输入框匹配度过低 ({input_box['confidence']:.3f} < 0.4)"
            )
        ...
        # ========== 步骤3: 连续点击三次激活输入框 ==========
        self.log(f"📋 步骤3: 连续点击三次激活输入框 ({input_cx}, {input_cy})...")
